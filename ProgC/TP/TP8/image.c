...
void ImLiberer(tImage *pIm, int Nblignes){
    MatLiberer(&((*pIm)->NivGris),Nblignes);
    free(*pIm);
    *pIm = NULL;
}
...
tImage ImLire(char NomFichier[]){
    ...
    ACTUALISER_TAMPON;
    PASSER_COMMENTAIRE;
    if (strncmp(Tampon,"P2",2)){
        #ifdef DEBUG
            printf("%c != 'P' ou/et %c != '2'\n",NomFichier[0],NomFichier[1]);
        #endif
        return NULL;
    }

    // Trouve la taille de la matrice
    int NbLig, NbCol;
    PASSER_COMMENTAIRE;
    ACTUALISER_TAMPON;
