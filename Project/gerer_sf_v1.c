...
static int TestCreerBloc(tBloc adresse[]){
...
static int TestDetruireBloc(tBloc adresse[]){
    for(int i = 0;i<NOMBRE_BLOC_TEST;i++){
...
static int TestEcrireContenuBloc(unsigned char** string){
...
static int TestLireContenuBloc(unsigned char** string){
    tBloc bloc = CreerBloc();
...
static int TestCreerInode(tInode adresse[]){
...
static int TestDetruireInode(tInode adresse[]){
    for(int i = 0;i<NOMBRE_INODE_TEST;i++){
...
static int TestNumero(tInode adresse[]){
    for (int i=0;i<3;i++){
...
static int TestDateDerAcces(tInode adresse[]){
    for (int i=0;i<3;i++){
        time_t temp;
        time(&temp);
...
static int TestDateDerModif(tInode adresse[]){
    for (int i=0;i<3;i++){
        time_t temp;
        time(&temp);
...
static int TestDateDerModifFichier(tInode adresse[]){
    for (int i=0;i<3;i++){
        time_t temp;
        time(&temp);
...
static int TestType(tInode adresse[]){
    for (int i=0;i<3;i++){
...
static int TestTaille(tInode adresse[]){
    for (int i=0;i<3;i++){
        time_t temp;
...
static int TestAfficherInode(void){
...
static int TestEcrireDonneesInode1bloc(void){
...
static int TestLireDonneesInode1bloc(void){
    tInode inode = CreerInode(0, 0);
    unsigned char string[23+1] ="Projet du module ProgC";
